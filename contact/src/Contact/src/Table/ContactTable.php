...
{
    public const DB_TABLE_NAME = 'contacts';
    ...
    {
        $data = $this->hydrator->extract($contactEntity);
    ...
    protected function selectMany(Select $select): ?HydratingResultSet
    {
        ...
        if ($result instanceof ResultInterface && $result->isQueryResult() && $result->count() > 0) {
            return (new HydratingResultSet($this->hydrator, new ContactEntity()))->initialize($result);
        }
